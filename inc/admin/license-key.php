...
class GT_Modern_License_Key {
	...
	static function activate_license() {
		...
		// Call the custom API.
		$response = wp_remote_post( GT_MODERN_STORE_API_URL, array(
			'timeout'   => 25,
			'sslverify' => true,
			'body'      => $api_params,
		) );
		...
		update_option( 'gt_modern_theme_options', $theme_options );

		delete_transient( 'gt_modern_license_check' );
	...
	static function check_license() {
		...
		$status = get_transient( 'gt_modern_license_check' );
		...
		if ( false === $status ) {
			...
			set_transient( 'gt_modern_license_check', $status, DAY_IN_SECONDS );
		}
